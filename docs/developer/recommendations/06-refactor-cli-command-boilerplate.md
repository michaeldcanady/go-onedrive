...
## Implementation plan
1.  **Identify Common Patterns**: Remove common setup logic like creating a logger for the command name and initializing a basic `CommandContext`
...
3.  **Refactor Existing Commands**: Update existing commands to use the new helper,  reducing the size of `command.go` files
