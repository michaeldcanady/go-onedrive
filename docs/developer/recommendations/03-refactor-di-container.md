...
2.  **Lazy Initialization**: Consider using a pattern where services initialize on first access or using a more robust DI library if the complexity warrants it (though manual provider functions are  sufficient for Go)
