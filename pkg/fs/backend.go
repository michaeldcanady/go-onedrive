...
type Backend interface {
	...
	List(ctx context.Context, token, driveID, path string) ([]Item, error)

	// Open returns an io.ReadCloser for the content of the file at the specified path.
	Open(ctx context.Context, token, driveID, path string) (io.ReadCloser, error)

	// Create uploads or updates a file with the content from the provided reader at the specified path.
	Create(ctx context.Context, token, driveID, path string, r io.Reader) (Item, error)

	// Mkdir creates a new directory at the specified path.
	Mkdir(ctx context.Context, token, driveID, path string) error

	// Remove deletes an item from the storage at the specified path.
	Remove(ctx context.Context, token, driveID, path string) error
...
type AdvancedBackend interface {
	...
	Move(ctx context.Context, token, driveID, src, dst string) error
