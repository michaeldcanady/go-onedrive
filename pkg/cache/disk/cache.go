...
var _ cache.Cache[any] = (*Cache[string, any])(nil)

type Cache[K comparable, V any] struct {
	path            string
	lock            *flock.Flock
	keySerializer   cache.SerializerDeserializer[K]
	valueSerializer cache.SerializerDeserializer[V]
...
func (c *Cache[K, V]) Set(ctx context.Context, key string, value V) error {
	...
	return c.setEntry(ctx, cache.NewEntry(k, value))
}
...
func (c *Cache[K, V]) readEntryAtOffset(offset int64, key K) (*cache.Entry[K, V], error) {
	...
	return cache.NewEntry(key, value), nil
}
