...
func (s *Service) listChildren(ctx context.Context, p string) ([]*infrafile.DriveItem, error) {
	driveID, err := s.driveResolver.CurrentDriveID(ctx)
	if err != nil {
		return nil, err
	}

	return s.files.ListChildren(ctx, driveID, p)
}
...
func (s *Service) List(ctx context.Context, p string, opts domainfs.ListOptions) ([]domainfs.Item, error) {
	...
	children, err := s.listChildren(ctx, p)
	if err != nil {
		return nil, err
	}
