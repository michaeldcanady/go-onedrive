...
type CachedFileContentsRepository struct {
	gateway       infrafile.GraphContentsGateway
	contentCache  infrafile.ContentsCache
...
func (r *CachedFileContentsRepository) Download(ctx context.Context, driveID, path string, opts file.DownloadOptions) (io.ReadCloser, error) {
	...
	if !opts.NoStore && ctag != "" {
		...
		_ = r.contentCache.Put(ctx, cacheKey, &file.Contents{
			CTag: ctag,
			Data: fresh,
		})
	}
...
func (r *CachedFileContentsRepository) Upload(ctx context.Context, driveID, path string, body io.Reader, opts file.UploadOptions) (*file.Metadata, error) {
	...
	if !opts.NoStore {
		_ = r.contentCache.Put(ctx, metadata.ID, &file.Contents{
			CTag: ctag,
			Data: data,
		})
