...
func TestDownloadCmd_Run(t *testing.T) {
	tests := []struct {
		name          string
		src           string
		dst           string
	...
	}{
		{
			...
			src:           "/remote.txt",
			dst:           "local.txt",
			mockContent:   "hello from onedrive",
			mockError:     nil,
			expectedError: false,
		},
		{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.mockError != nil {
				mockFS.On("ReadFile", mock.Anything, tt.src, mock.Anything).Return(nil, tt.mockError)
			} else {
				r := io.NopCloser(strings.NewReader(tt.mockContent))
				mockFS.On("ReadFile", mock.Anything, tt.src, mock.Anything).Return(r, nil)
			}

			cmd := NewDownloadCmd(mockContainer)
			opts := Options{
				Source:      tt.src,
				Destination: tt.dst,
				Stdout:      io.Discard,
			...
			if tt.expectedError {
				...
				content, _ := os.ReadFile(tt.dst)
				assert.Equal(t, tt.mockContent, string(content))
			}
		})
	}
}
