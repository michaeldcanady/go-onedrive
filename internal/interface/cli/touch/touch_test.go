...
func TestTouchCmd_Run(t *testing.T) {
	tests := []struct {
		name          string
		path          string
		mockError     error
		...
		},
		{
			name:          "error",
			path:          "/fail",
			mockError:     errors.New("touch failed"),
			expectedError: true,
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			mockFS.On("Touch", mock.Anything, tt.path, mock.Anything).Return(domainfs.Item{}, tt.mockError)
