...
func TestDriveListCmd_Run(t *testing.T) {
	tests := []struct {
		name          string
		mockDrives    []*domaindrive.Drive
		mockError     error
		expectedError bool
	}{
		{
			name: "success",
			mockDrives: []*domaindrive.Drive{
				{ID: "1", Name: "Drive 1", Type: domaindrive.DriveTypePersonal},
			},
			mockError:     nil,
			expectedError: false,
		},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			mockDrive.On("ListDrives", mock.Anything).Return(tt.mockDrives, tt.mockError)
			if tt.mockError == nil {
				mockState.On("Get", domainstate.KeyDrive).Return("1", nil)
				mockState.On("ListDriveAliases").Return(map[string]string{"my": "1"}, nil)
			}
