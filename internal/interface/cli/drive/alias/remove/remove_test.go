...
func (m *MockStateService) RemoveDriveAlias(alias string) error {
	args := m.Called(alias)
	return args.Error(0)
}
...
func TestDriveAliasRemoveCmd_Run(t *testing.T) {
	tests := []struct {
		name          string
		alias         string
		mockError     error
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			mockState.On("RemoveDriveAlias", tt.alias).Return(tt.mockError)

			cmd := NewRemoveCmd(mockContainer)
			opts := Options{
				Alias:  tt.alias,
				Stdout: io.Discard,
			}
