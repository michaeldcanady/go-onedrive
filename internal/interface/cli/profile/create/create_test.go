...
func TestProfileCreateCmd_Run(t *testing.T) {
	tests := []struct {
		name          string
		profileName   string
		force         bool
		...
		},
		{
			...
			profileName:   "fail",
			exists:        false,
			createError:   errors.New("disk full"),
			expectedError: true,
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			mockProfileSvc.On("Exists", mock.Anything, tt.profileName).Return(tt.exists, nil)
			if tt.exists && tt.force {
				mockProfileSvc.On("Delete", mock.Anything, tt.profileName).Return(nil)
			}

			if !tt.exists || tt.force {
				mockProfileSvc.On("Create", mock.Anything, tt.profileName).Return(domainprofile.Profile{Name: tt.profileName}, tt.createError)
			}

			cmd := NewCreateCmd(mockContainer)
			opts := Options{
				Name:       tt.profileName,
				Force:      tt.force,
				SetCurrent: tt.setCurrent,
				Stdout:     io.Discard,
			}
