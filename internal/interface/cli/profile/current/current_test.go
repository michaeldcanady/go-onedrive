...
func TestProfileCurrentCmd_Run(t *testing.T) {
	tests := []struct {
		name          string
		mockProfile   string
		mockError     error
		...
		},
		{
			name:          "error",
			mockProfile:   "",
			mockError:     errors.New("state error"),
			expectedError: true,
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			mockState.On("Get", domainstate.KeyProfile).Return(tt.mockProfile, tt.mockError)

			cmd := NewCurrentCmd(mockContainer)
			opts := Options{
				Stdout: io.Discard,
			}
