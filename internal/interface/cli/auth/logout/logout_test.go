...
func (m *MockAuthService) Logout(ctx context.Context, profile string, force bool) error {
	args := m.Called(ctx, profile, force)
	return args.Error(0)
}
...
func TestLogoutCmd_Run(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			mockAuth.On("Logout", mock.Anything, tt.profile, tt.force).Return(tt.mockError)

			cmd := NewLogoutCmd(mockContainer)
			opts := Options{
				Force:  tt.force,
				Stdout: io.Discard,
			}
