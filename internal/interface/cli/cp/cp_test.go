...
func TestCpCmd_Run(t *testing.T) {
	tests := []struct {
		name          string
		src           string
		dst           string
		...
		{
			name: "error folder without recursive",
			src:  "/folder",
			dst:  "/folder-copy",
			mockItem: domainfs.Item{
				Path: "/folder",
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			mockFS.On("Get", mock.Anything, tt.src).Return(tt.mockItem, tt.getError)
			if tt.getError == nil && (tt.mockItem.Type != domainfs.ItemTypeFolder || tt.recursive) {
				mockFS.On("Copy", mock.Anything, tt.src, tt.dst, mock.Anything).Return(tt.copyError)
			}

			cmd := NewCpCmd(mockContainer)
			opts := Options{
				Source:    tt.src,
				Dest:      tt.dst,
