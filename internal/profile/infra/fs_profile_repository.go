...
func (s *FSProfileService) List() ([]domainprofile.Profile, error) {
	if err := s.ensureDefault(); err != nil {
		return nil, err
	}
	...
	for _, e := range entries {
		if e.IsDir() {
			profiles = append(profiles, domainprofile.Profile{
				Name: e.Name(),
...
func (s *FSProfileService) Create(name string) (domainprofile.Profile, error) {
	if err := s.ensureDefault(); err != nil {
	...
	return domainprofile.Profile{Name: name, Path: p}, nil
}
...
func (s *FSProfileService) Get(_ context.Context, name string) (domainprofile.Profile, error) {
	if err := s.ensureDefault(); err != nil {
		return domainprofile.Profile{}, err
	}
	...
	if !exists {
		return domainprofile.Profile{}, fmt.Errorf("profile %q does not exist", name)
	}
	return domainprofile.Profile{Name: name, Path: s.profilePath(name)}, nil
}
