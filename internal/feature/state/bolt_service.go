...
func NewBoltService(env environment.Service) (*BoltService, error) {
	...
	dbFilePath := filepath.Join(dbPath, "state.db")
	...
	if _, err := bs.Get(KeyProfile); err != nil {
		_ = bs.Set(KeyProfile, profile.DefaultProfileName, ScopeGlobal)
	}
...
func (bs *BoltService) ensureBuckets() error {
	return bs.db.Update(func(tx *bolt.Tx) error {
		...
		if _, err := tx.CreateBucketIfNotExists([]byte("drive_aliases")); err != nil {
...
func (bs *BoltService) Get(key Key) (string, error) {
	...
	err := bs.db.View(func(tx *bolt.Tx) error {
		...
		for _, scopeKey := range [][]byte{[]byte("session"), []byte("global")} {
			b := tx.Bucket(scopeKey)
			if b == nil {
				return fmt.Errorf("bucket %s not found", string(scopeKey))
			}
		...
		return fmt.Errorf("state key %s not found in any scope", keyStr)
	})
...
func (bs *BoltService) Clear(key Key) error {
	return bs.db.Update(func(tx *bolt.Tx) error {
		...
		for _, bucketName := range [][]byte{[]byte("session"), []byte("global")} {
...
func (bs *BoltService) SetDriveAlias(alias, driveID string) error {
	return bs.db.Update(func(tx *bolt.Tx) error {
		b, err := tx.CreateBucketIfNotExists([]byte("drive_aliases"))
		if err != nil {
