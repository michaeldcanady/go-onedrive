...
func TestMetadataListCacheAdapter_Get(t *testing.T) {
	...
	tests := []struct {
		...
		cacheGetVal   any
		expectOK      bool
		expectListing *domain.Listing
	}{
		{
			name:        "hit: valid listing",
			path:        "/foo/listing",
			cacheGetErr: nil,
			cacheGetVal: domain.Listing{ETag: "etag123", ChildIDs: []string{"a", "b"}},
			expectOK:    true,
			expectListing: &domain.Listing{
				ETag:     "etag123",
				ChildIDs: []string{"a", "b"},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			mockCache := new(MockCache2[domain.Listing])

			mockCache.
				On("Get", mock.Anything, tt.path).
				Return(tt.cacheGetVal.(domain.Listing), tt.cacheGetErr)

			adapter := NewMetadataListingCacheAdapter(mockCache)
