...
func NewGraphMetadataGateway(client abstractions.RequestAdapter, l domainlogger.Logger) *GraphMetadataGateway {
...
func (g *GraphMetadataGateway) GetByPath(ctx context.Context, driveID, path string, etag string) (*domainfs.Metadata, error) {
...
func (g *GraphMetadataGateway) ListByPath(ctx context.Context, driveID, path string, parentEtag string) ([]*domainfs.Metadata, error) {
	...
	metadatas := make([]*domainfs.Metadata, 0, len(realItems))
...
func (g *GraphMetadataGateway) CreateByPath(ctx context.Context, driveID, parentPath string, request domainfs.MetadataCreateRequest) (*domainfs.Metadata, error) {
	requestBody := models.NewDriveItem()
	name := request.Name
	requestBody.SetName(&name)

	switch request.Type {
	case domainfs.ItemTypeFolder:
		folder := models.NewFolder()
		requestBody.SetFolder(folder)
	case domainfs.ItemTypeFile:
		file := models.NewFile()
...
func (g *GraphMetadataGateway) UpdateByPath(ctx context.Context, driveID, path string, request domainfs.MetadataUpdateRequest) (*domainfs.Metadata, error) {
	requestBody := models.NewDriveItem()
	if request.Name != "" {
