...
func NewProvider(l domainlogger.Logger) *Provider {
	return &Provider{
		log: l,
	}
}

func (p *Provider) buildLogger(ctx context.Context) domainlogger.Logger {
...
func (p *Provider) Get(ctx context.Context, path string) (domainfs.Item, error) {
	log := p.buildLogger(ctx).With(domainlogger.String("path", path))
	log.Debug("local.Get")
...
func (p *Provider) List(ctx context.Context, path string, opts domainfs.ListOptions) ([]domainfs.Item, error) {
	log := p.buildLogger(ctx).With(domainlogger.String("path", path))
	log.Debug("local.List", domainlogger.Bool("recursive", opts.Recursive))
...
func (p *Provider) WriteFile(ctx context.Context, path string, r io.Reader, opts domainfs.WriteOptions) (domainfs.Item, error) {
	log := p.buildLogger(ctx).With(domainlogger.String("path", path))
	log.Debug("local.WriteFile", domainlogger.Bool("overwrite", opts.Overwrite))
...
func (p *Provider) Mkdir(ctx context.Context, path string, opts domainfs.MKDirOptions) error {
	log := p.buildLogger(ctx).With(domainlogger.String("path", path))
	log.Debug("local.Mkdir", domainlogger.Bool("parents", opts.Parents))
...
func (p *Provider) Copy(ctx context.Context, src, dst string, opts domainfs.CopyOptions) error {
	log := p.buildLogger(ctx).With(domainlogger.String("src", src), domainlogger.String("dst", dst))
