...
func TestMkdirCmd_Run(t *testing.T) {
	tests := []struct {
		name          string
		path          string
		parent        bool
		...
		},
		{
			...
			path:          "/fail",
			parent:        false,
			mockError:     errors.New("mkdir failed"),
			expectedError: true,
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			mockFS.On("Mkdir", mock.Anything, tt.path, domainfs.MKDirOptions{Parents: tt.parent}).Return(tt.mockError)

			cmd := NewCmd(mockContainer)
			opts := Options{
				Path:   tt.path,
				Parent: tt.parent,
				Stdout: io.Discard,
