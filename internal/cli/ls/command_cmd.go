...
func NewLsCmd(container didomain.Container) *LsCmd {
	return &LsCmd{
...
func (c *LsCmd) Run(ctx context.Context, opts Options) error {
	...
	c.Log.Info("starting ls command",
		domainlogger.String("format", opts.Format),
		domainlogger.Bool("includeAll", opts.IncludeAll),
		domainlogger.Bool("foldersOnly", opts.FoldersOnly),
		domainlogger.Bool("filesOnly", opts.FilesOnly),
		domainlogger.String("sortProperty", opts.SortProperty),
		domainlogger.String("sortDirection", opts.SortOrder.String()),
	...
	c.Log.Info("ls command completed",
		...
		domainlogger.Int("finalItemCount", len(items)),
	)
