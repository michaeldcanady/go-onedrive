...
func (m *mockIdentityRepository) Save(ctx context.Context, provider string, token AccessToken) error {
	return m.Called(ctx, provider, token).Error(0)
}
func (m *mockIdentityRepository) Get(ctx context.Context, provider string, accountID string) (AccessToken, error) {
...
func (m *mockIdentityRepository) Close() error { return m.Called().Error(0) }
...
func TestIdentityService_Functional(t *testing.T) {
	...
	tests := []struct {
		name    string
		setup   func(mAuth *mockAuthenticator, mStore *mockIdentityRepository)
