...
func TestGetCommand_Integration(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			args: []string{"get", "personal"},
			setup: func(m *mockContainer, mSvc *mockDriveService, mLogSvc *mockLoggerService, mLog *mockLogger) {
				mLogSvc.On("CreateLogger", "drive-get").Return(mLog, nil)
				m.On("Logger").Return(mLogSvc)
				m.On("Drive").Return(mSvc)

				mSvc.On("ResolveDrive", mock.Anything, "personal", "").Return(drive.Drive{
		...
		{
			name: "get drive with identity",
			args: []string{"get", "personal", "--id", "user@example.com"},
			setup: func(m *mockContainer, mSvc *mockDriveService, mLogSvc *mockLoggerService, mLog *mockLogger) {
				mLogSvc.On("CreateLogger", "drive-get").Return(mLog, nil)
				m.On("Logger").Return(mLogSvc)
				m.On("Drive").Return(mSvc)

				mSvc.On("ResolveDrive", mock.Anything, "personal", "user@example.com").Return(drive.Drive{
					ID: "d1", Name: "Personal Drive", Type: "personal",
				}, nil)
