...
func TestRmCommand_Integration(t *testing.T) {
	tests := []struct {
		name    string
		args    []string
		...
		{
			name: "rm successfully",
			args: []string{"rm", "/od/file"},
			setup: func(m *mockContainer, mFS *mockFsService, mLogSvc *mockLoggerService, mLog *mockLogger, mVFS *mockVFS) {
				mLogSvc.On("CreateLogger", "rm").Return(mLog, nil)
				...
				mFS.On("Remove", mock.Anything, mock.Anything).Return(nil)
			},
			wantErr: false,
		},
	}
