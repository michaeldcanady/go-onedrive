...
func TestRm_Functional(t *testing.T) {
	tests := []struct {
		name    string
		path    string
	...
	}{
		{
			...
			setup: func(m *mockBackend) {
				...
				m.On("Remove", mock.Anything, mock.Anything, mock.Anything, "/file.txt").Return(nil)
			},
			wantErr: false,
		},
		{
			name: "remove failed - not found",
			path: "/od/nonexistent.txt",
			setup: func(m *mockBackend) {
				m.On("IdentityProvider").Return("")
				m.On("Remove", mock.Anything, mock.Anything, mock.Anything, "/nonexistent.txt").
