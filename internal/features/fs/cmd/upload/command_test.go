...
func TestUploadCommand_Integration(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			args: []string{"upload", "/local/src", "/od/dst"},
			setup: func(m *mockContainer, mFS *mockFsService, mLogSvc *mockLoggerService, mLog *mockLogger, mVFS *mockVFS) {
				mLogSvc.On("CreateLogger", "upload").Return(mLog, nil)
				m.On("Logger").Return(mLogSvc)
				m.On("FS").Return(mFS)
				m.On("URIFactory").Return(registry.NewURIFactory(mVFS))
				mVFS.On("Resolve", mock.Anything).Return("/local/src", "/", nil)
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			cmd := CreateUploadCmd(mContainer)
