...
func TestCatCommand_Integration(t *testing.T) {
	tests := []struct {
		name    string
		args    []string
		...
		{
			name: "cat successfully",
			args: []string{"cat", "od:/file"},
			setup: func(m *mockContainer, mFS *mockFsService, mLogSvc *mockLoggerService, mLog *mockLogger, mVFS *mockVFS) {
				mLogSvc.On("CreateLogger", "drive-cat").Return(mLog, nil)
				m.On("Logger").Return(mLogSvc)
				m.On("FS").Return(mFS)
				m.On("URIFactory").Return(registry.NewURIFactory(mVFS))
				mVFS.On("Resolve", mock.Anything).Return("od:/file", "/", nil)
				mLog.On("WithContext", mock.Anything).Return(mLog); mLog.On("With", mock.Anything).Return(mLog)
				mLog.On("Info", mock.Anything, mock.Anything).Return()
				mLog.On("Debug", mock.Anything, mock.Anything).Return()
				mFS.On("ReadFile", mock.Anything, mock.Anything, mock.Anything).Return(io.NopCloser(bytes.NewBufferString("content")), nil)
			},
			wantErr: false,
		},
	}
