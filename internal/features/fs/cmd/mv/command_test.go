...
func TestMvCommand_Integration(t *testing.T) {
	tests := []struct {
		name    string
		args    []string
		...
		{
			name: "mv successfully",
			args: []string{"mv", "/od/src", "/od/dst"},
			setup: func(m *mockContainer, mFS *mockFsService, mLogSvc *mockLoggerService, mLog *mockLogger, mVFS *mockVFS) {
				mLogSvc.On("CreateLogger", "mv").Return(mLog, nil)
				...
				mFS.On("Move", mock.Anything, mock.Anything, mock.Anything).Return(nil)
			},
			wantErr: false,
		},
	}
