...
func (m *mockContainer) URIFactory() *registry.URIFactory { return m.Called().Get(0).(*registry.URIFactory) }
...
func (m *mockFsService) Get(ctx context.Context, uri *pkgfs.URI) (pkgfs.Item, error) { args := m.Called(ctx, uri); return args.Get(0).(pkgfs.Item), args.Error(1) }
func (m *mockFsService) List(ctx context.Context, uri *pkgfs.URI, opts pkgfs.ListOptions) ([]pkgfs.Item, error) { args := m.Called(ctx, uri, opts); return args.Get(0).([]pkgfs.Item), args.Error(1) }
func (m *mockFsService) ReadFile(ctx context.Context, uri *pkgfs.URI, opts pkgfs.ReadOptions) (io.ReadCloser, error) { args := m.Called(ctx, uri, opts); return args.Get(0).(io.ReadCloser), args.Error(1) }
func (m *mockFsService) Stat(ctx context.Context, uri *pkgfs.URI) (pkgfs.Item, error) { args := m.Called(ctx, uri); return args.Get(0).(pkgfs.Item), args.Error(1) }
func (m *mockFsService) WriteFile(ctx context.Context, uri *pkgfs.URI, r io.Reader, opts pkgfs.WriteOptions) (pkgfs.Item, error) { args := m.Called(ctx, uri, r, opts); return args.Get(0).(pkgfs.Item), args.Error(1) }
func (m *mockFsService) Mkdir(ctx context.Context, uri *pkgfs.URI) error { return m.Called(ctx, uri).Error(0) }
func (m *mockFsService) Touch(ctx context.Context, uri *pkgfs.URI) (pkgfs.Item, error) { args := m.Called(ctx, uri); return args.Get(0).(pkgfs.Item), args.Error(1) }
func (m *mockFsService) Remove(ctx context.Context, uri *pkgfs.URI) error { return m.Called(ctx, uri).Error(0) }
func (m *mockFsService) Copy(ctx context.Context, src, dst *pkgfs.URI, opts pkgfs.CopyOptions) error { return m.Called(ctx, src, dst, opts).Error(0) }
func (m *mockFsService) Move(ctx context.Context, src, dst *pkgfs.URI) error { return m.Called(ctx, src, dst).Error(0) }

type mockVFS struct{ mock.Mock }
func (m *mockVFS) Resolve(absPath string) (string, string, error) { args := m.Called(absPath); return args.String(0), args.String(1), args.Error(2) }

func TestLsCommand_Integration(t *testing.T) {
	tests := []struct {
		name    string
		args    []string
	...
	}{
		{
			...
			setup: func(m *mockContainer, mFS *mockFsService, mLogSvc *mockLoggerService, mLog *mockLogger, mVFS *mockVFS) {
				...
				mFS.On("List", mock.Anything, mock.Anything, mock.Anything).Return([]pkgfs.Item{}, nil)
			},
			wantErr: false,
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			// Hack: Inject stdout directly into the cmd.Options via some reflection or by reaching into it if possible.
			// Actually the command handler logic uses ctx.Options.Stdout = cmd.OutOrStdout() in PreRunE, 
            // but the test executes cmd.Execute which runs PreRunE. 
            // The issue is that the formatter is created in Execute() and it takes ctx.Options.Stdout.
            // Let's ensure cmd.SetOut() is called, which it is.
