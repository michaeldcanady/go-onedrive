...
func TestTouch_Functional(t *testing.T) {
	tests := []struct {
		name    string
		path    string
	...
	}{
		{
			...
			path: "/od/new-file.txt",
			setup: func(m *mockBackend) {
				m.On("IdentityProvider").Return("")
				m.On("Create", mock.Anything, mock.Anything, mock.Anything, "/new-file.txt", mock.Anything).
					Return(fs.Item{}, nil)
			},
			wantErr: false,
		},
		{
			name: "touch failed - permission denied",
			path: "/od/readonly.txt",
			setup: func(m *mockBackend) {
				m.On("IdentityProvider").Return("")
				m.On("Create", mock.Anything, mock.Anything, mock.Anything, "/readonly.txt", mock.Anything).
					Return(fs.Item{}, errors.New("permission denied"))
			},
			wantErr: true,
		},
	}
