...
func TestCp_Functional(t *testing.T) {
	tests := []struct {
		name        string
		source      string
		destination string
	...
	}{
		{
			...
			setup: func(m *mockBackend) {
				...
				m.On("Copy", mock.Anything, mock.Anything, mock.Anything, "/src.txt", "/dst.txt").Return(nil)
			},
			wantErr: false,
		},
		{
			...
			setup: func(m *mockBackend) {
				...
				m.On("Open", mock.Anything, mock.Anything, mock.Anything, "/src.txt").
					Return(io.NopCloser(bytes.NewBufferString("content")), nil)
				m.On("Create", mock.Anything, mock.Anything, mock.Anything, "/dst.txt", mock.Anything).
					Return(fs.Item{}, nil)
			},
			wantErr: false,
		},
		{
			...
			setup: func(m *mockBackend) {
				m.On("IdentityProvider").Return("")
				m.On("Capabilities").Return(fs.Capabilities{CanCopy: true})
				m.On("Copy", mock.Anything, mock.Anything, mock.Anything, "/nonexistent.txt", "/dst.txt").
