...
func TestGetCommand_Integration(t *testing.T) {
	tests := []struct {
		name    string
		args    []string
		setup   func(m *mockContainer, mSvc *mockConfigService, mLogSvc *mockLoggerService, mLog *mockLogger)
		wantErr bool
	}{
		{
			name: "config get success",
			args: []string{"get", "auth.provider"},
			setup: func(m *mockContainer, mSvc *mockConfigService, mLogSvc *mockLoggerService, mLog *mockLogger) {
				mLogSvc.On("CreateLogger", "config-get").Return(mLog, nil)
				m.On("Logger").Return(mLogSvc)
				m.On("Config").Return(mSvc)
				
				mSvc.On("GetConfig", mock.Anything).Return(config.Config{
					Auth: config.AuthenticationConfig{Provider: "test"},
				}, nil)
